...
////////////////////////////////////////////////////////////////////////
//
// File:        OpBoundaryProcessv2Process.cc
...
OpBoundaryProcessv2::OpBoundaryProcessv2(const G4String& processName,
                                         G4ProcessType type)
...
OpBoundaryProcessv2::~OpBoundaryProcessv2()
{}
 ...
 *
 */
bool OpBoundaryProcessv2::CheckDetection(const G4Track& theTrack, const G4Step& theStep)
{
    ...
    majsim::PrimaryGeneratorAction* genAction = g4Helper->GetActionInitialization()->GetGeneratorAction();

    G4double mppcHalfL     = config->MPPCHalfLength();
    G4double diskRadius    = config->DiskRadius();

    std::vector<G4ThreeVector> mppcPositions;
    ...
    G4double y     = thePosition.y();
    G4double z     = thePosition.z();
         ...
         (mppcZ-mppcHalfL) <= z && z <= (mppcZ+mppcHalfL) )
    {
        ...
        // Convert photon position to polar
        double tanTheta = std::abs(y/x);
        theta = std::atan(tanTheta)*180*deg/pi;
        if      (x >= 0 && y >= 0) theta = theta;
        else if (x <  0 && y >= 0) theta = 180*deg - theta;
        else if (x <  0 && y <  0) theta += 180*deg;
        else if (x >= 0 && y <  0) theta  = 360*deg - theta;

        unsigned mppc(1);
        for (const auto& pos : mppcPositions)
        {
            // Get theta
            G4double thisTheta = pos[1];
            // Handle special case when mppc == 1
            //if (mppc ==1 ) G4cout << thisTheta/deg << " " << theta/deg << " " << delT/deg << G4endl;
            bool mppc1CaseBool = (mppc == 1) && (theta <= delT || theta >= (360*deg-delT));
            if ( mppc1CaseBool ||
...
G4VParticleChange*
OpBoundaryProcessv2::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
{
    theStatus = Undefined;
    ...
    //****************************************
    // ADDED
    if (CheckDetection(aTrack, aStep))
    {
    ...
    else
    {
        ...
        ed << " OpBoundaryProcessv2Process/PostStepDoIt(): "
           << " The Navigator reports that it returned an invalid normal"
           << G4endl;
        G4Exception("OpBoundaryProcessv2::PostStepDoIt", "OpBoun01",
    ...
    ed << G4endl;
    G4Exception("OpBoundaryProcessv2::PostStepDoIt", "OpBoun02",
...
void OpBoundaryProcessv2::BoundaryProcessVerbose() const
{
...
G4ThreeVector
OpBoundaryProcessv2::GetFacetNormal(const G4ThreeVector& Momentum,
...
void OpBoundaryProcessv2::DielectricMetal()
{
    G4int n = 0;
...
void OpBoundaryProcessv2::DielectricLUT()
{
    ...
    theStatus = OpBoundaryProcessv2ProcessStatus(G4int(theFinish) +
...
void OpBoundaryProcessv2::DielectricLUTDAVIS()
{
...
void OpBoundaryProcessv2::DielectricDichroic()
{
    // Calculate Angle between Normal and Photon Momentum
    G4double anglePhotonToNormal = OldMomentum.angle(-theGlobalNormal);
    ...
    if (DichroicVector) {
        ...
        G4ExceptionDescription ed;
        ed << " OpBoundaryProcessv2Process/DielectricDichroic(): "
           << " The dichroic surface has no G4Physics2DVector"
           << G4endl;
        G4Exception("OpBoundaryProcessv2Process::DielectricDichroic", "OpBoun03",
                    FatalException,ed,
...
void OpBoundaryProcessv2::DielectricDielectric()
{
    G4bool Inside = false;
    G4bool Swap   = false;
...
G4double OpBoundaryProcessv2::GetMeanFreePath(const G4Track& ,
                                              G4double ,
...
G4double OpBoundaryProcessv2::GetIncidentAngle()
{
...
G4double OpBoundaryProcessv2::GetReflectivity(G4double E1_perp,
                                              G4double E1_parl,
...
void OpBoundaryProcessv2::CalculateReflectivity()
{
...
G4bool OpBoundaryProcessv2::InvokeSD(const G4Step* pStep)
{
    G4Step aStep = *pStep;
