#define makeOpRefTable2618_cxx
#include "makeOpRefTable2618.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
...
std::string pixelizationPath = "/home/hunter/projects/Majorana/production/2618Pixels_v1/pixelization.txt";
class Pixel;
...
unsigned nEvents(15);
Int_t factor(0); // used to switch nSiPMs, nSiPMs --> nSiPMs / 2^(factor)
...
{ 
  ...
  // Write to file
  // pixelID mppcID prob
  std::string name = std::to_string(pixelList.size())+"p_"+std::to_string(modNSiPMs)+"s_opReferenceTable.txt";
