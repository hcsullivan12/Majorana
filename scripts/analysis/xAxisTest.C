...
void xAxisTest::Loop()
{
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) 
  {
    ...
    if (isnan(mppc1Dist)) mppc1Dist = diskRadius;
    if (isnan(mppc5Dist)) mppc5Dist = diskRadius;
    std::cout << e << " " << mppc5Dist << std::endl;

   //if (mppc5Dist < 3)  std::cout << "5 " << mppc5Dist << " " << mppc5LY << std::endl;
...
/*  TCanvas* c1 = new TCanvas("c1", "c1", 800, 800);
  ...
  for (const auto& xbin : mppc5Map)
  {
    ...
    for (unsigned ybin = 1; ybin <= data.size(); ybin++)
    {
      if (xbin.first < 1) std::cout << data[ybin] << std::endl;
      hist.Fill(data[ybin]);
    }
    Double_t mean  = hist.GetMean();
    Double_t stdev = hist.GetStdDev();
  ...
  //mppc1->SetMarkerStyle(8);
  //mppc5->SetMarkerStyle(8);
