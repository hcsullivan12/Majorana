#define angleNoDiskAna_cxx
#include "angleNoDiskAna.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
...
std::vector<float> vec1(141, 0);
std::vector<float> vec2(141, 0);
...
void angleNoDiskAna::Loop()
{
  ...
  for (const auto& v : voxelList)
  {
    ...
    for (unsigned s = 1; s <= 4; s++) sipmLYMap.emplace(s, 0);
