...
def writeNewOpRefTable(avgMap, pixelTable, outputFile):
	...
	with open(outputFile, 'w') as f:
		# write first line
		f.write('pixelID x y mppcID probability\n')
		for pid in sorted(newPixels.iterkeys()):
			s = str(pid)+' '+str(newPixels[pid][0])+' '+str(newPixels[pid][1])+' 1 '+str(newPixels[pid][2])+'\n'
			f.write(s) 

if __name__ == "__main__":
	# first read the input op ref table
	parser = argparse.ArgumentParser(description="Make Spline from OpReferenceTable")
	parser.add_argument("-orf", "--opreftable", help="The input OpRefTable", required=True)
	parser.add_argument("-p", "--pixelization", help="The pixelization scheme", required=True)
